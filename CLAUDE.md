...
2. **RF transmission / provision mode (loop):** When `provisionName != ""`, every 1 second the loop calls `mySwitch.disableReceive()`, transmits `provisionCode` (24-bit, generated by `esp_random()`) via `RF_TX_PIN`, then re-enables receive. This runs for `PROVISION_DURATION_MS` (15 s). The code is saved to `users.json` immediately when provision starts, before transmission begins.

3. **Web server (loop):** `WebServer` handles HTTP requests. Every route calls `requireAuth()` first, which validates the `session` cookie against the in-RAM sessions array.

4. **Relay timing (loop):** Non-blocking — `relayEndMs` is set when the relay activates; the loop turns it off when `millis() >= relayEndMs`.
...
```cpp
#define RF_RX_PIN             27    // 433MHz DATA pin (receiver)
#define RF_TX_PIN             25    // 433MHz DATA pin (FS1000A transmitter)
